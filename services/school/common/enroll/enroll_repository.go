...
func (repository *Repository) GetAll(filter *types.Filter, pagination *types.Pagination, typeName string) (result []model.School, err error) {
	...
	var where string = ""
	if len(typeName) > 0 {
		where = fmt.Sprintf("WHERE enrolls.type = '%s'", typeName)
	}
	if filter != nil && len(filter.Search) >= 1 {
		tempWhere := fmt.Sprintf(
			"CAST(enrolls.id AS TEXT) = '%s' OR enrolls.name ILIKE '%s' OR CAST(enrolls.type AS TEXT) ILIKE '%s' OR infos.full_name ILIKE '%s' OR infos.slogan ILIKE '%s' OR infos.founder ILIKE '%s'",
			filter.Search,
			"%"+filter.Search+"%",
			"%"+filter.Search+"%",
			"%"+filter.Search+"%",
	...
	newFilter := filter
	newFilter.OrderBy = "enrolls." + newFilter.OrderBy
	tmpErr := repository.Db.Preload(clause.Associations).Scopes(
		helpers.PaginationScope(
			repository.Db,
			"SELECT enrolls.id, enrolls.name, enrolls.type, enrolls.logo, enrolls.currency, enrolls.enroll_config_id, enrolls.enroll_info_id"+
				", enrolls.created_at, enrolls.updated_at FROM enrolls "+
				"LEFT JOIN enroll_infos as infos ON enrolls.enroll_info_id = infos.id",
			where,
			pagination,
			newFilter,
		),
