...
func (repository *Repository) GetAllLevelClass(filter *types.Filter, pagination *types.Pagination, studentID int64) (result []model.StudentLevelClass, err error) {
	...
	if filter != nil && len(filter.Search) >= 1 {
		tempWhere := fmt.Sprintf(
			"CAST(student_lc.id AS TEXT) = '%s' OR years.name ILIKE '%s' OR levels.name ILIKE '%s' OR levels.description ILIKE '%s' OR classes.name ILIKE '%s' OR classes.description ILIKE '%s'",
			filter.Search,
			"%"+filter.Search+"%",
			"%"+filter.Search+"%",
			"%"+filter.Search+"%",
