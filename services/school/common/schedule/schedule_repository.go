	...
	err = repository.Db.
		...
		Scopes(
			helpers.PaginationScopeV2(
				...
				LEFT JOIN teacher_class_subject_units ON schedules.school_id = teacher_class_subject_units.school_id
				AND teacher_class_subject_units.year_id = schedules.year_id
				AND (
				(teacher_class_subject_units.class_subject_id IS NOT NULL AND schedules.class_subject_id = teacher_class_subject_units.class_subject_id)
				OR
				(teacher_class_subject_units.unit_id IS NOT NULL AND schedules.unit_id = teacher_class_subject_units.unit_id)
				)
				
				LEFT JOIN student_enrolls ON schedules.school_id = student_enrolls.school_id
				AND student_enrolls.year_id = schedules.year_id
				AND (
