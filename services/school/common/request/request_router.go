...
func RegisterEndpoints(
	...
	huma.Register(
		*humaApi,
		huma.Operation{
			...
			Description: "Update existing request with matching id and return the new request object.",
	...
	huma.Register(
		*humaApi,
		huma.Operation{
			...
			Description: "Delete existing request with matching id and return affected rows in database.",
	...
	huma.Register(
		*humaApi,
		huma.Operation{
			OperationID: "delete-request-multiple",
			Summary:     "Delete multiple request",
			Description: "Delete multiple request by providing a lis of IDs and return affected rows in database.",
			Method:      http.MethodDelete,
	...
	huma.Register(
		*humaApi,
		huma.Operation{
			...
			Description: "Get all request with support for search, filter and pagination",
			Method:      http.MethodGet,
			Path:        endpointConfig.Group,
			Tags:        endpointConfig.Tag,
			Security: []map[string][]string{
		...
		func(
			...
			tempResults := make([]data.RequestResponse, 10)
			for i := range tempResults {
				tmpModel := data.RequestResponse{}
				tmpModel.ID = int64(i)
