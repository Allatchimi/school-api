	...
	err = repository.Db.
		...
		Scopes(
			helpers.PaginationScopeV2(
				...
				LEFT JOIN teacher_class_subject_units ON quizzes.school_id = teacher_class_subject_units.school_id
				AND teacher_class_subject_units.year_id = quizzes.year_id
				AND (
				(teacher_class_subject_units.class_subject_id IS NOT NULL AND quizzes.class_subject_id = teacher_class_subject_units.class_subject_id)
				OR
				(teacher_class_subject_units.unit_id IS NOT NULL AND quizzes.unit_id = teacher_class_subject_units.unit_id)
				)

				LEFT JOIN student_enrolls ON quizzes.school_id = student_enrolls.school_id
				AND student_enrolls.year_id = quizzes.year_id
				AND (
